...
const PokemonDetailsPage = ({ match }: RouteComponentProps<MatchParams>) => {
  ...
  const transitions = useTransition(activeTab, (p: any) => p, {
    from: { opacity: 0 },
    enter: { opacity: 1 },
    leave: { opacity: 0 },
    config: {
  ...
  const selectedPokemon = pokemons.data.find(
    (pokemon: any) => pokemon !== null && pokemon.id === Number(id)
  );
  const selectedSpecies = species.data.find((s: any) => s.id === Number(id));
  const evolutionChainId =
    selectedSpecies?.evolutionChain?.url.split("/").slice(-2)[0] || null;
  const selectedEvolutionChain =
    evolutionChainId !== null
      ? evolutionChain.data.find((e: any) => e.id === Number(evolutionChainId))
      : null;
